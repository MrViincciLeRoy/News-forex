...
class HFAnomalyDetector:
    ...
    def set_baseline(self, historical_data: pd.DataFrame, symbol: str):
        ...
        stats = {
            ...
            'mean_range': (historical_data['high'] - historical_data['low']).mean(),
            'std_range': (historical_data['high'] - historical_data['low']).std()
        }
        
        historical_data['returns'] = historical_data['close'].pct_change()
        stats['mean_returns'] = historical_data['returns'].mean()
        stats['std_returns'] = historical_data['returns'].std()
        stats['volatility'] = historical_data['returns'].std() * np.sqrt(252)
    ...
    def detect_price_anomaly(self, current_price: float, symbol: str) -> Dict:
        ...
        return {
            ...
            'baseline_mean': round(stats['mean_price'], 2),
            'deviation': round(current_price - stats['mean_price'], 2),
            'severity': severity,
    ...
    def detect_volume_anomaly(self, current_volume: int, symbol: str) -> Dict:
        ...
        return {
            ...
            'multiplier': round(current_volume / stats['mean_volume'], 2),
            'severity': severity,
    ...
    def detect_volatility_regime_change(self, recent_data: pd.DataFrame, 
        ...
        is_anomaly = vol_ratio > 1.5 or vol_ratio < 0.5
    ...
    def comprehensive_anomaly_scan(self, current_data: pd.Series,
        ...
        price_check = self.detect_price_anomaly(current_data['close'], symbol)
        ...
        volume_check = self.detect_volume_anomaly(current_data['volume'], symbol)
