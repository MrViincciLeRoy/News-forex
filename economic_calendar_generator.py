...
class GoldIndicatorCalculator:
    ...
    async def fetch_fred_gold_data(self, start_date='2001-01-01', end_date=None):
        ...
        if not self.fred_key:
            print("⚠️  FRED_API_KEY not set, skipping FRED")
            return None
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, timeout=30) as response:
                    print(f"FRED response status: {response.status}")
                    if response.status == 200:
                        ...
                        print(f"FRED error response: {error_text[:200]}")
    ...
    async def load_and_cache_gold_data(self, start_year, end_year):
        ...
        print(f"\n{'='*60}")
        print(f"Loading Gold Data ({start_date} to {end_date})")
        print(f"{'='*60}")
        print(f"Alpha Vantage keys available: {len(self.alpha_keys)}")
        print(f"FRED key available: {'Yes' if self.fred_key else 'No'}")
        print(f"{'='*60}\n")
        ...
        if df is None and self.alpha_keys:
            print("\nFRED failed, trying Alpha Vantage...")
            df = await self.fetch_alpha_vantage_gold(start_date)
        
        if df is not None:
            print(f"\n✓ Successfully loaded {len(df)} days of gold data")
            print("Calculating indicators...")
            self.gold_cache = self.calculate_indicators(df)
            print(f"✓ Indicators calculated for {len(self.gold_cache)} days\n")
            return True
        
        print("\n✗ Failed to load gold data from all sources\n")
        return False
